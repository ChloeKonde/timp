...
class Tree {
  ...
  insert(data, newOffset) {
    const newNode = new Node(data, newOffset);

    if (this.root === null) {
      this.root = newNode;
      this.offset = newOffset;
    } else this.insertNode(this.root, newNode, newOffset);
  }
  ...
  insertNode(node, newNode, newOffset) {
    if (newNode.data < node.data) {
      ...
      } else this.insertNode(node.left, newNode, newOffset);
    } else if (node.right == null) {
      node.right = newNode;
      node.offset = newOffset;
    } else this.insertNode(node.right, newNode, newOffset);
  }
...
stream.on("open", function (fd) {
  for (let i = 0; i < 10000; i++) {
    const hashes = generateHashes(36);
  ...
  stream.on("finish", () => {
    ...
    fs.readFileSync("data.txt")
      ...
      .forEach(function (line, index, arr) 
      {
        ...
        if (line.includes(hs[index])) findings++; // used for checking that this works
      });
    console.timeEnd("Execution Time");
  });
});
